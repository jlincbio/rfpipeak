! npPsi14.f90, version 01/27/2017
! Standalone version of NuPoP for rfPIPeak (human, 4th order HMM)
! Code modified to accept a list of FASTA files; designed to be streamlined with PSI main program
! Compiler version tested: GNU Fortran 6.3.0
! Parameter: npPsi14 <fasta_count> <fasta_list>
! Recommended compiler options: gfortran -O2 npPsi14.f90 -o npPsi14

! References: Xi L et. al, Predicting nucleosome positioning using a duration Hidden Markov Model, 
!             BMC Bioinformatics (2010) 11:346,
! 			  with parameter files and main() based on the Memorial Sloan Kettering Cancer Center
!			  Galaxy implementation of the NuPoP source code for the R package.
!			  GitHub URL: vipints/NuPoP-GX
program main
  implicit none
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  integer i,lfn,mlL,rep,species,order; character*80 listFasta,fileCount,fastaFile; character*3 tpc
  real*8  freqL1(4),tranL1(4,4),tranL2(16,4),tranL3(64,4),tranL4(256,4),Pd(500,11)
  real*8  freqN4(64,4),tranN4((147-4)*256,4)
  integer errF,maxCount,k
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  
  if(command_argument_count()==2) then
	  call get_command_argument(1,fileCount)
	  call get_command_argument(2,listFasta)
  else
	  call exit(1)
  end if
  listFasta=trim(listFasta)
  fileCount=trim(fileCount)
  errF=0; read(fileCount,*,iostat=errF) maxCount
  
  ! HMM model definitions
  freqL1 = (/0.33853, 0.16147, 0.16147, 0.33853/)
  open(1,file='npParameters/npptL.prm')
  do i=1,4; read(1,*) tranL1(i,:); end do; close(1)
  open(1,file='npParameters/npptL2.prm')
  do i=1,16; read(1,*) tranL2(i,:); end do; close(1)
  open(1,file='npParameters/npptL3.prm')
  do i=1,64; read(1,*) tranL3(i,:); end do; close(1)
  open(1,file='npParameters/npptL4.prm')
  do i=1,256; read(1,*) tranL4(i,:); end do; close(1)
  open(1,file='npParameters/nppfN4.prm')
  do i=1,64; read(1,*) freqN4(i,:); end do; close(1)
  open(1,file='npParameters/npptN4.prm')
  do i=1,(147-4)*256; read(1,*) tranN4(i,:); end do; close(1)
  open(1,file='npParameters/nppPd.prm')
  do i=1,500; read(1,*) Pd(i,1:11); end do; close(1)
	  
  rep=1; species=1; order=4; mlL=500
  errF=1; open(91,file=listFasta,status='old',err=9); errF=0
9 if(errF==1) then
    call exit(1)
  end if
  
  do k=1,maxCount
    read(91,'(a80)',iostat=errF) fastaFile
	lfn=len_trim(fastaFile)
	call vtbfbNL4(lfn,trim(fastaFile),freqL1,tranL1,tranL2,tranL3,tranL4,freqN4,tranN4,mlL,rep,species,Pd)
  end do
  close(91)
  call exit(0)
end
...
subroutine vtbfbNL4(lfn,filename,freqL1,tranL1,TtranL2,TtranL3,TtranL4,TfreqN4,TtranN4,mlL,rep,species,Pdd)

  implicit none
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  integer   t,d,i,j,k,l,m,n,z,mlL,rep,lfn,nvt,species,tep,lfn1
  ...
  allocate(a(1))
  l=1; open(1,file=filename,status='old',err=99); l=0
99 if(l==1) then
    call exit(1)
  end if
  ...
  if(ztt<148) then
	...
	rep=1
	!Updating linker length distribution is not allowed since the sequence is not long enough.
    !The linker length distribution is still assumed uniform.
  ...
  lfn1=lfn-i
  out1(1:lfn1)=filename(i+1:lfn); out1(lfn1+1:lfn1+10)='_nupop.txt'
  open(1,file=out1(1:lfn1+16),status='replace'); temp=-0.05; t=0; tpc='0'
  do i=1,nrcv(1)-1; write(1,'(i9,2x,f5.2,3x,f5.2,3x,i1,4x,a)') i,temp,temp,t,tpc; end do
  close(1)

  ! human specific definitions (species == 1)
  scc(1,1)=0.9690; scc(2,1)=1.050
  sc(1)=scc(1,species); sc(2)=scc(2,species)
  sc(3)=sc(2); sc(4)=sc(1)
  tep=species; Pd(1:mlL)=Pdd(1:mlL,tep)
  !--------------- scaling ---------------
  do k=1,4
